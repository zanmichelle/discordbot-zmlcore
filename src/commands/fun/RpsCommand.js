...
module.exports = class RpsCommand extends BaseCommand {
    ...
    async run(client, message, args) {
        try{
            ...
            if(d==valid[0] && bd == valid[1]) return message.channel.send(embed.setDescription(`**I chose 📄 so I've won!**`))
            else if(d==valid[1] && bd == valid[2]) return message.channel.send(embed.setDescription(`**I chose ✂️ so I've won!**`))
            else if(d==valid[2] && bd == valid[0]) return message.channel.send(embed.setDescription(`**I chose ⛰ so I've won!**`))
            else return message.channel.send(embed.setDescription(`I chose \`${bd}\` so **you've won**!`));
